...
свои достоинства. В основном он полезен в контексте больших 
приложений со множеством компонент.

Представьте интерфейс панели администрации с множеством
виджетов и страниц с разными состояниями. Представьте
продукт, который должен оставаться стабильным с учётом того,
...
* Логика именования позволяет сразу понять, что именно
  значит CSS-класс.

* Каждый элемент создаёт собственное пространство имён, что
...
* Имена классов могут стать длинноватыми и страшноватыми (Но
   мне всё равно, потому что они обладают «логикой» которую
...
* Фреймворки, такие как Bootstrap, не основаны на БЭМ, так
  что их сложно комбинировать, что, в свою очередь, влечёт
  потерю целостности пространства имён, что может стать
  причиной путаницы.

Как по мне, для сайтов, которые поддерживают несколько, или
